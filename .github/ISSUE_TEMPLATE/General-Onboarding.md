# TRUST General Onboarding Checklist

## Day 1 (Led by New Team Member)
- [ ] Accept Slack invite and join relevant project channels  
    - [ ] [#shared-irs-general](https://slack.com/archives/C08TTH9MW6M) (shared-irs-general)
    - [ ] [#shared-irs-pub-17](https://slack.com/archives/C08TUFZ3EKB) (shared-irs-pub-17)
    - [ ] [#shared-irs-design](https://slack.com/archives/C08TTHLJDS5)  (shared-irs-design)
    - [ ] [#shared-irs-engineering](https://slack.com/archives/C08T6HDUWRZ) (shared-irs-engineering)
    - [ ] [#shared-irs-product](https://slack.com/archives/C08THT8CJVA) (shared-irs-product)
    - [ ] [#tax-witholding-estimator](https://slack.com/archives/C08V3R4HSRJ) (tax-withholding-estimator) - if applicable
    - [ ] [#shared-irs-tax-logic-standup8266](https://slack.com/archives/C08UWGW7HPS)  (shared-irs-tax-logicstandup8266) - if applicable
    - [ ] [#twe-design](https://slack.com/archives/C09EYBS9LGJ) (twe-design) - if applicable
    - [ ] [#twe_flow ](https://slack.com/archives/C09A48XDRNC)(twe_flow) - if applicable
    - [ ] [#twe_logic](https://slack.com/archives/C099ANCQMLN) (twe_logic) - if applicable
- [ ] Accept GitHub and Figma team invites and verify you have access
    - [ ] Access to the IRSDigitalService project
    - [ ] Access to the following repositories:
        - [ ] trust
        - [ ] tax-withholding-estimator
        - [ ] fact-graph
        - [ ] internal-assets
        - [ ] twe-prototype
        - [ ] mef-sdk  

## Team Context (Led by Team Lead / Product Manager)

- [ ] Assign buddy
- [ ] Walk through TWE 2.0 vision, strategy, and goals (why this matters, how it improves taxpayer experience).
- [ ] Share the roadmap and milestone plan (M1, M2, M3), including where we are today.
- [ ] Explain the relationship between TWE 1.0 and 2.0 (why 1.0 is paused, focus shifted to 2.0).
- [ ] Walk through current timelines for TWE
- [ ] Explain team structure and roles
- [ ] Review IRS stakeholders and decision makers
- [ ] Invite to all-team meetings:
    - [ ] TRUST All Hands (every week, Monday 2:30 pm)
    - [ ] Retro (Week 1, Tuesday 3 pm)
    - [ ] Sprint planning (Week 1, Wednesday, 2 pm)
- [ ] Share team norms and working agreements
    - [ ] Use Slack threads to keep conversations organized.
    - [ ] Document decisions in epics and notes for visibility.
    - [ ] Celebrate wins with team recognition posts (“Hurrah” culture).
    - [ ] Ask clarifying questions early and often. We're figuring it out together!

## Social Onboarding (Led by Buddy)

- [ ] Introduce to all team members (with context on their roles)
- [ ] Invite to TRUST Social Club
- [ ] Schedule 1:1s with key team members
    - [ ] Engineering Lead
    - [ ] Design Lead
    - [ ] Product Managers

## Once you have access to PIV and GFE (Led by New Team Member)

- [ ] Confirm with your project lead that Nicole is the assigned approver in BEARs and that she has added the ERAP entitlement
- [ ] Confirm your PIV works and you can log in
- [ ] Request BEARS entitlements, list depends on if you are Product, UX or Engineering
    - [ ] Product:
    - [ ] UX: Figma
    - [ ] Engineering:

